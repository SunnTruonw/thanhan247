...
@section('title',"Sửa about")

@section('css')
@endsection
@section('content')
<div class="content-wrapper lb_template_about_edit">
   @include('admin.partials.content-header',['name'=>"about","key"=>"Edit about"])
   <!-- Main content -->
   <div class="content">
      <div class="container-fluid">
         <div class="row">
            <div class="col-md-12">
               ...
               @endif
               <form action="{{route('admin.about.update',['id'=>$data->id])}}" method="POST" enctype="multipart/form-data">
                  @csrf
                  <div class="row">
                    <div class="col-md-4">
                ...
                <div class="row">
                    <div class="col-md-8">
                        <div class="card card-outline card-primary">
                            <div class="card-header">
                               <h3 class="card-title">Thông tin about</h3>
                            </div>
                            <div class="card-body table-responsive p-3">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="date"
                                        ...
                                        value="{{ old('created_date')??$data->created_date  }}"
                                        name="created_date" >
                                    @error('created_date')
                                        <div class="invalid-feedback d-block">{{ $message }}</div>
                                    @enderror
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control custom-select select-2-init @error('admin_id')
                                        is-invalid
                                        @enderror" id="" value="{{ old('admin_id')??$data->admin_id }}" name="admin_id" data-title="Chọn phóng viên">

                                        <option value="">--- Chọn phóng viên ---</option>
                                        @foreach ($admins as $admin)
                                        <option value="{{ $admin->id }}" {{ (old('admin_id')??$data->admin_id)== $admin->id?'selected':'' }}>{{ $admin->name }}</option>
                                        @endforeach
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea
                                        ...
                                        value="{{  old('description' )??$data->description  }}" placeholder="Nhập mô tả">
