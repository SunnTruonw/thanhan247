...
namespace App\Http\Requests\Admin\CategoryProgram;

use Illuminate\Foundation\Http\FormRequest;
use Illuminate\Validation\Rule;

class ValidateEditCategoryProgram extends FormRequest
{
    /**
    ...
    public function rules()
    {
        ...
        foreach ($langConfig as $key => $value) {
            ...
            $pro = \App\Models\CategoryProgram::find($idPro)->translationsLanguage($key)->first();
            $id = optional($pro)->id;
            ...
            $rule['slug_' . $key] = [
                "required",
                'different:' . $stringKey,
                Rule::unique("App\Models\CategoryProgramTranslation", 'slug')->ignore($id, 'id'),
            ];
        }
        return $rule;
    }
