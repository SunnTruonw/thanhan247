...
namespace App\Http\Requests\Admin\CategoryExam;

use Illuminate\Foundation\Http\FormRequest;
use Illuminate\Validation\Rule;

class ValidateEditCategoryExam extends FormRequest
{
    /**
    ...
    public function rules()
    {
        ...
        foreach ($langConfig as $key => $value) {
            ...
            $pro = \App\Models\CategoryExam::find($idPro)->translationsLanguage($key)->first();
            $id = optional($pro)->id;
            ...
            $rule['slug_' . $key] = [
                "required",
                'different:' . $stringKey,
                Rule::unique("App\Models\CategoryExamTranslation", 'slug')->ignore($id, 'id'),
            ];
        }
        return $rule;
    }
